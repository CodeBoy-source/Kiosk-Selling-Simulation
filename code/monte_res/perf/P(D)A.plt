...
do for[opt in optimization]{
    do for [file in fileposfix]{
        name="P(D)".file.opt
        filename = name.".txt"
        if (file_exists(filename)) {
            stats filename every ::::-1 u 1:2 nooutput
            set title "Distribución de probabilidad P(".file.") en caso ".opt
            print "Targetting: ".filename
            outname = dir."/".name.".png"
            set output outname
            plot filename u :2 w lp lw 1.5 t "Distribución"
        }
    }
}
