...
# set key outside;
set key right top;
# set key spacing 2 font "Helvetica, 14"
...
do for[file in fileposfix]{
        ...
        if (file_exists(filename)) {
            set title "Tiempos de cómputo para las distintos implementaciones"
            print "Targetting: ".filename
            outname = dir."/".name.".png"
            set output outname
            plot for[opt in optimization] name.opt.".txt" u :2 w lp lw 1.5 t "Tiempo de cómputo caso ".opt
        }
    }
